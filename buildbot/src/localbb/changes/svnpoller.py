...
class SVNPoller(base.ChangeSource, util.ComparableMixin):
    ...
    def __init__(self, svnurl, split_file=None,
        ...
        @param svnbin:       path to svn binary, defaults to just 'svn'. Use
                             ...
                             unusual location.
        @type  listener: tuple (interface{string}, port{int}, signature{string})
        @param listener: Parameters for starting CVSListenerFactory.
                         Defaults to None.
    ...
    def startService(self):
        ...
        if self.listener:
            reactor.listenTCP(self.listener[1],
                              SVNListenerFactory(self, self.listener[2]),
                              interface=self.listener[0])

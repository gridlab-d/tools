% This script should be run after VoltageData_CircuitMiles has output
% "voltage_distance_data.mat"
%  - It will plot circuit miles versus voltage (on 120 V basis)
%  - It will also produce a "movie" or series of plots if you have multiple
%    times periods recorded (my_ind)
%  - You can turn on or off the secondary voltages if the graphic is too
%    busy
% 
...
% Directory and file you want to load
dir = '';%'C:\Users\d3x289\Documents\LAAR\3.1\VS2005\x64\Release\DR Run 01 14 2015\';
load([dir 'voltage_distance_data.mat']);

% Plot the secondary voltages or not -- w/ secondary is sometimes very busy
primary_only = 0; % 1=TRUE, 0=FALSE

% If you have multiple runs you want to string together, change this index
% to include all that you want.
my_ind = 1:1;
...
for kind=my_ind
    ...
    for jind=1:length(data.pu_a)
            ...
            if (data.pu_a(jind) ~= 0)
                if (data.volt_length(jind) > 0)
                    if (data.parent_index(jind) ~= 0)
                        if ( data.pu_a(data.parent_index(jind)) == 0)
        ...
        elseif (data.phases(jind) == 1 && primary_only == 0)
            if (data.volt_length(jind) > 0)
                if (data.parent_index(jind) ~= 0)
                    if ( data.pu_a(data.parent_index(jind)) == 0)
                        plot(data.volt_length(jind)/5280,data.pu_a(jind),'-ok','MarkerSize',2);
                    else
    ...
    for jind=1:length(data.pu_b)
        if (data.volt_pu(jind) > 240)
            if (data.pu_b(jind) ~= 0)
                if (data.volt_length(jind) > 0)
                    if (data.parent_index(jind) ~= 0)
                        if ( data.pu_b(data.parent_index(jind)) == 0)
            ...
            if (data.volt_length(jind) > 0)
                if (data.parent_index(jind) ~= 0)
                    if (data.phases(data.parent_index(jind)) == 0)
                        ...
                        if ( data.pu_a(data.parent_index(jind)) == 0)
                            plot(data.volt_length(jind)/5280,data.pu_a(jind),':.g','MarkerSize',2);
                        else
    ...
    for jind=1:length(data.pu_c)
        if (data.volt_pu(jind) > 240)
            if (data.pu_c(jind) ~= 0)
                if (data.volt_length(jind) > 0)
                    if ( data.pu_c(data.parent_index(jind)) == 0)
                        plot(data.volt_length(jind)/5280,data.pu_c(jind),'-ok','MarkerSize',2);
                    elseif (data.parent_index(jind) ~= 0)
        ...
        elseif (data.phases(jind) == 3 && primary_only == 0)
            if (data.volt_length(jind) > 0)
                if (data.parent_index(jind) ~= 0)
                    if (data.phases(data.parent_index(jind)) == 0)
                        if ( data.pu_c(data.parent_index(jind)) == 0)
                            plot(data.volt_length(jind)/5280,data.pu_c(jind),':.k','MarkerSize',2);
                        else

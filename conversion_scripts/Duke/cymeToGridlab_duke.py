...
def convertCymeModel(network_db, equipment_db, switchID, test=False, type=1, feeder_id=None, primaryLoadOnly=True):
    if (test==False):
    ...
    ugl_cfgs = {}
    ugls = {}
    uglcfgDict = _readUndergroundCableGLMFile()
            ...
            if ugl not in cymundergroundline.keys():
                ...
                if config_name not in ugl_cfgs.keys():
                    ...
                    ugl_cfgs[config_name] = {'object' : 'line_configuration',
                                                                'name' : config_name}
                    for key, value in thisUglcfgDict.iteritems():
                        # if key[0] == 'c':
                        #     pass
                        # else:
    ...
    switch = switchID + ',2000-01-01 00:00:05,2000-01-01 00:00:30'
    if primaryLoadOnly:
        voltdumpFilename = 'Voltage.csv'
    else:
        voltdumpFilename = 'Voltage_triplex.csv'
    eventgen={ 'object' : 'eventgen',
                    'name' : 'testgendev',
                    'fault_type' : '\"SW-ABC\"',
                    'manual_outages' : '\"' + switch + '\"'}
    voltdump={'object' : 'voltdump',
                    'filename' : '\"' + voltdumpFilename + '\"'}
    objectList = [{'key':fault_check}, {'key':eventgen}, {'key':voltdump}, \
                  #{'key':impedance_dump}, \
                  ohl_conds, ugl_conds, ohl_spcs, ugl_sps, \
                  ohl_cfgs, ugl_cfgs, xfmr_cfgs, spct_cfgs, reg_cfgs, meters, nodes, loads, tpms, tpns, ohls, ugls, xfmrs, spcts, regs, swObjs, rcls, sxnlrs, fuses, caps]
...
def _readUndergroundCableGLMFile():
    try:
        dictSource = open("Underground Cable.glm", 'r')
    except IOError:
        warnings.warn('An error occured trying to read the \'Underground Cable.glm\' file.')
    else:
...
def _tests(database, feederID, switchID, keepFiles=True, primaryLoadOnly=True, ugConfig=''):
    ...
    try:
        db_network = database
        db_equipment = database
        cyme_base, x, y = convertCymeModel(db_network, db_equipment, switchID, test=False, type=1, feeder_id=feederID,
                                           primaryLoadOnly=primaryLoadOnly)
        glmString = feeder.sortedWrite(cyme_base)
        if primaryLoadOnly:
            gfile = open("./uploads/" + feederID + ".glm", 'w')
        else:
            gfile = open("./uploads/" + feederID + "_Triplex.glm", 'w')
        gfile.write(glmString)
        gfile.close()
        print 'WROTE GLM FOR'
        outPrefix = './uploads/' + feederID + '/'
        try:
            os.mkdir(outPrefix)
        except:
        ...
        try:
            ...
            plt.savefig(outPrefix + feederID + "_summer_allocate.png")
            print 'DREW GLM OF'
        except:
            exceptionCount += 1
        ...
        try:
            ...
            with open(outPrefix + feederID + "_summer_allocate.JSON",'w') as outFile:
...
if __name__ == '__main__':
    if len(sys.argv) == 5:
        ...
        feedId1 = sys.argv[3]
        switchID = sys.argv[4]
        _tests(database, feedId1, switchID, primaryLoadOnly=True, ugConfig=ugConfig)
    else:
        warnings.warn("Usage: python cymeToGridlab_duke.py databaseFullPath undergroundConfigGLMFilePath feederID1 switchIDfeederadfd")
        exit()

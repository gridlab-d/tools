...
if len(sys.argv) != 4:
    ...
    try:
        ...
        with open(cymeFile, 'r') as cymeFile:
            ...
            for row in cymeLines:  # type: object
                if row[0] != 'Node Id ' and row[0] != '':
                #     pass
                # else:
                    cymeDict[row[0]] = {'A' : row[16], 'B' : row[17], 'C' : row[18]}
    except IOError:
    ...
    try:
        ...
        exit()
    for nodeName, nodeValue in gridLabDDict.items():
        try:
    ...
    with open(outputFilename, 'w', newline='') as csvfile:
        ...
        for key, value in matchedNodes.items():
        ...
        for key, value in unMatchedNodes.items():
        ...
        uniqueGridLabDNodes = []
        for key, value in gridLabDDict.items():
        ...
        uniqueCYMENodes = []
        for key, value in cymeDict.items():
    ...
    print("There are {nodeNumber} nodes in GridLAB-D model exist in CYME model.\n" \
          "Voltage A: \n" \
          "\tmax: {VAMax:.10%}" \
          "\tmin: {VAMin:.10%}" \

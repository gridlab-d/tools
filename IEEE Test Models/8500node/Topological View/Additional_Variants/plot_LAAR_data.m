...
if regulatorCapacitorTaps == 1
    ...
    tapsDR0 = sum(abs(diff(data.DR0.regulator_taps(3600/timestampInterval:end,:))));
    tapsDR2 = sum(abs(diff(data.DR2.regulator_taps(3600/timestampInterval:end,:))));
    tapsDR3 = sum(abs(diff(data.DR3.regulator_taps(3600/timestampInterval:end,:))));
    ...
    tapsDR0 = sum(abs(diff(data.DR0.capacitor_state(3600/timestampInterval:end,:))));
    tapsDR2 = sum(abs(diff(data.DR2.capacitor_state(3600/timestampInterval:end,:))));
    tapsDR3 = sum(abs(diff(data.DR3.capacitor_state(3600/timestampInterval:end,:))));
    ...
    disp('Capacitor 3');
    disp('  Phase             A         B         C');
    disp(['     DR0             ' num2str(tapsDR0(7)) '         ' num2str(tapsDR0(8)) '             N/A']);
    disp(['     DR2             ' num2str(tapsDR2(7)) '         ' num2str(tapsDR2(8)) '             N/A']);
    disp(['     DR3             ' num2str(tapsDR3(7)) '         ' num2str(tapsDR3(8)) '             N/A']);
    disp('Capacitor 3');
    disp('  Phase             A         B         C');
    disp(['     DR0             ' num2str(tapsDR0(9)) '             N/A             N/A' ]);
    disp(['     DR2             ' num2str(tapsDR2(9)) '             N/A             N/A' ]);
    disp(['     DR3             ' num2str(tapsDR3(9)) '             N/A             N/A']);
    disp(' ');     
end
...
if voltageMapPlot == 1
    ...
    volt_to_plot_DR0 = volt_DR0.mag(voltage_point-37,:);
    ...
    figure();
    subplot(1,3,1)
    %scatter([volt.X(indexDR3) minX-1 minX-1],[volt.Y(indexDR3) minY-1 minY-1],8,[volt_to_plot_DR0(indexDR3) minV maxV],'+');
    scatter([volt.X minX-1 minX-1],[volt.Y minY-1 minY-1],8,[volt_DR0.mag(voltage_point,:) minV maxV],'o');
    hold on
    scatter(VREG.X,VREG.Y,70,'r','o','filled');
    scatter(CAPBANKS.X,CAPBANKS.Y,70,'b','o','filled');
